...
const STATUS_CONFIG = {
  ACTIVE:    { label: 'Activa',     icon: CheckCircle, cls: 'bg-green-500/15 text-green-400 border-green-500/20' },
...
export default function AdminMovies() {
  ...
  useEffect(() => {
    const loadMultiplexes = async () => {
      ...
      finally { setLoadingMultiplexes(false) }
    }
    loadMultiplexes()
  }, [])
  ...
  const onMultiplexChange = async (e) => {
    ...
    if (!plex) { setScreeningForm(f => ({ ...f, multiplexName: '', multiplexId: '', roomId: '' })); setRooms([]); return }
    setScreeningForm(f => ({ ...f, multiplexName: plex.nameMultiplex, multiplexId: plex.idMultiplex, roomId: '' }))
    try {
  ...
  const handleCreateScreening = async () => {
    ...
    if (!selectedMovie.id) {
      setScreeningError('Selecciona una película válida en la pestaña "Buscar".')
      return
    }
  ...
  return (
    <AdminLayout>
      ...
      <div className="mb-8 animate-[fadeUp_0.5s_ease-out_forwards]">
        ...
        <p className="text-text-secondary mt-2">Gestión de cartelera y programación de funciones</p>
      </div>
      ...
      {tab === 'buscar' && (
        <div className="animate-[fadeUp_0.4s_ease-out_forwards]">
          ...
          <div className="relative mb-8 max-w-xl">
            ...
            />
            {searching && (
              <Loader2 size={18} className="absolute right-4 top-1/2 -translate-y-1/2 text-magenta animate-spin" />
            )}
          </div>
                ...
                >
                  ...
                  <div className="relative aspect-[2/3] bg-carbon overflow-hidden">
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end p-3">
                      ...
                      >
                        {selectingId === movie.id ? <Loader2 size={14} className="animate-spin mx-auto" /> : 'Seleccionar'}
                      </button>
                    </div>
                  </div>
          ...
          {!query && (
            <div className="text-center py-20 text-text-secondary">
              ...
              <p className="text-lg">Escribe el título de una película para buscarla</p>
              <p className="text-sm mt-2 opacity-60">Búsqueda en tiempo real vía TMDB</p>
            </div>
          )}
        </div>
      )}
        ...
        <div className="max-w-2xl animate-[fadeUp_0.4s_ease-out_forwards]">
          {!selectedMovie && (
            <div className="flex items-center gap-3 bg-yellow-500/10 border border-yellow-500/30 text-yellow-400 rounded-2xl px-5 py-4 mb-6">
              <AlertCircle size={18} />
              Primero busca y selecciona una película en la pestaña <strong>Buscar Película</strong>.
            </div>
          )}
          ...
          <div className="bg-surface/50 border border-border/50 rounded-3xl p-6 space-y-5">
            {/* Multiplex */}
            <div>
              <label className="block text-xs font-bold tracking-widest text-text-secondary mb-2 uppercase">Multiplex</label>
              {loadingMultiplexes ? (
                <div className="flex items-center gap-2 text-text-secondary text-sm py-3">
                  <Loader2 size={14} className="animate-spin" /> Cargando multiplex...
                </div>
              ) : (
                <select
                ...
                >
                  ...
                  {multiplexes.map(m => (
                    <option key={m.idMultiplex} value={m.idMultiplex}>{m.nameMultiplex}</option>
                  ))}
                </select>
              )}
            </div>
            ...
            {/* Sala */}
            <div>
              <label className="block text-xs font-bold tracking-widest text-text-secondary mb-2 uppercase">Sala</label>
              <select
                value={screeningForm.roomId}
                onChange={(e) => setScreeningForm(f => ({ ...f, roomId: e.target.value }))}
              ...
              >
                <option value="">
                  {rooms.length === 0 ? 'Selecciona un multiplex primero' : 'Seleccionar sala'}
                </option>
                {rooms.map(r => (
                  <option key={r.idRoom} value={r.idRoom}>
                    Sala #{r.numberRoom} — {r.seats?.totalAvailable ?? '?'} asientos disponibles
                  </option>
                ))}
              </select>
            </div>
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setScreeningForm(f => ({ ...f, dateTime: e.target.value }))}
                className="w-full bg-carbon border border-border/50 rounded-2xl px-4 py-3 outline-none focus:border-magenta text-white transition-colors"
              />
            </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  onChange={(e) => setScreeningForm(f => ({ ...f, price: e.target.value }))}
            ...
            {createdScreening && (
              <div className="bg-green-500/10 border border-green-500/30 rounded-2xl px-5 py-4">
                ...
                <p className="text-text-secondary text-sm mt-1">
                  ID: <span className="text-white font-mono text-xs">{createdScreening.screeningId}</span>
                </p>
              </div>
            )}
            ...
            >
              {creatingScreening ? <Loader2 size={18} className="animate-spin" /> : <Clapperboard size={18} />}
              Crear Función
            </button>
          </div>
        </div>
        ...
        <div className="animate-[fadeUp_0.4s_ease-out_forwards]">
          ...
          {screenings.length === 0 ? (
            <div className="text-center py-24 text-text-secondary">
              ...
              <p className="text-lg">No hay funciones creadas en esta sesión.</p>
              ...
              <div className="overflow-x-auto w-full max-w-full">
                <table className="w-full min-w-[800px] text-left border-collapse">
                  ...
                  <tbody className="divide-y divide-border/30">
                    {screenings.map(sc => {
                      const cfg = STATUS_CONFIG[sc.status] || STATUS_CONFIG.ACTIVE
                      const Icon = cfg.icon
                      return (
                        <tr key={sc.screeningId} className="hover:bg-carbon/40 transition-colors">
                          ...
                          <td className="py-4 px-4">
                            <span className={`flex items-center gap-1.5 w-fit px-2.5 py-1 rounded-full text-xs font-bold border ${cfg.cls}`}>
                          ...
                          <td className="py-4 px-4">
                            <div className="relative">
                              <select
                                value={sc.status}
                                onChange={(e) => handleStatusChange(sc, e.target.value)}
                              ...
                              {updatingId === sc.screeningId
                                ? <Loader2 size={12} className="absolute right-2.5 top-1/2 -translate-y-1/2 animate-spin text-magenta" />
